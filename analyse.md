...
|---|---|---|
| 2026-08-05 | Professional logo (flat redesign) | Rebuilt the JobVerse identity as a flat, geometric mark per the design brief: an abstract **"J"** (white rounded stroke) combined with a **cyan upward arrow** (career growth) and a **cyan checkmark** (successful placement) on a solid indigo `#4F46E5` tile (no gradients/shadows). Wordmark uses **Inter**: "Job" bold `#111827` + "Verse" medium `#4F46E5`. Updated `src/components/Logo.tsx` (mark + wordmark, dark-mode safe) and the favicon. Deliverables in `public/logo/`: `icon.svg` (icon-only), `horizontal.svg` (icon + wordmark), `app-icon.svg` (circular), `monochrome.svg` (single indigo), `white.svg` (all white for dark backgrounds), plus `public/favicon.svg`. |
| 2026-08-05 | Profile photo uploads | Every user (student, recruiter, admin) can upload a profile photo. New `avatars` storage bucket (public, per-user `{user-id}/` folders, RLS in `supabase/migrations/20260805_profile_avatars.sql`). `src/lib/api.ts` gained `uploadAvatar` / `removeAvatar` (upload → save public URL to `profiles.avatar` → refresh via `updateProfile`). New reusable `src/components/AvatarWithUpload.tsx` (camera button + remove button, image-only, ≤2MB) replaces the decorative camera icon on the Student and Recruiter profile pages. The avatar image is now shown in the Sidebar account card and the TopNav avatar too (initials fallback). Admin panel lists now include `avatar` in their profile queries (`fetchAllStudents`, `fetchAllRecruiters`, `fetchAllApplications`) and render the photo in the Students, Recruiters, and Applications tables (initials fallback). |
| 2026-08-05 | Dark-mode visibility fixes (My Resume) | The "Upload Resume" card is now legible in dark mode: the uploaded-file card uses `bg-green-50` (covered by the dark override) instead of `bg-green-50/50` (translucent light green that washed out over the dark card); the neutral Download/Replace buttons use `bg-slate-100 dark:bg-slate-700` so they don't blend into the page background; the "PDF only · Max 5MB" hint uses `text-[#9CA3AF]`. The banner CTAs ("Generate AI Resume" / "Download PDF") use `bg-slate-50 text-slate-900` instead of `!bg-white text-indigo-700`, which dark mode was overriding to a dark button. Global dark overrides improved: `text-[#CBD5E1]` → `#64748B` (was `#475569`, near-invisible) and added `bg-green-100` for dark. |
| 2026-08-05 | Relative date fixes | `formatDate` in `src/lib/utils.ts` now compares **calendar days** instead of elapsed 24h spans, so "Today"/"Yesterday"/"Nd ago" advance correctly at midnight (previously an application made late at night stayed "Today" until 24h had passed). It also handles missing/invalid timestamps (returns `—` instead of `Invalid Date`) and future dates (shows the calendar date instead of `-Nd ago`). Job cards on the Browse Jobs page now render "Posted …" for clarity. |
| 2026-08-05 | One-time AI Resume Builder | The builder is now usable **once per student**. On generate, the AI resume is persisted immediately. The builder locks if the student has **ever** created an `ai_generated` resume (`fetchAiResume` queries for any AI resume, not just the latest row — uploading a PDF later can't bypass it) and is replaced by a locked screen ("You've already created your AI resume") with a Download PDF button (renders the saved AI content) and a link to My Resume. Server-side, a `prevent_duplicate_ai_resume` trigger (`supabase/migrations/20260805_one_ai_resume_per_user.sql`) raises an error on any second `ai_generated` insert for the same student, so the rule can't be bypassed even via direct API calls. After generating, a prominent amber notice warns: *Download your resume now — the AI Resume Builder is one-time only and you won't be able to generate it again.* The success view shows "Saved to your account" once persisted. On the My Resume page, the "Generate AI Resume" CTA is replaced by a "Your AI resume is ready / Download PDF" banner once an AI resume exists (the JobDetail apply modal only offers AI generation when the student has no resume at all). |
